    ...
    for (uint8 i = 0; i < 3; ++i)
    {
        ...
        for (uint8 j = EQUIPMENT_SLOT_START; j < EQUIPMENT_SLOT_END; ++j)
        {
                ...
                if (pItem->GetProto()->RequiredSpell == reqSpell)
            {
                ...
                sLog->outDebug("TSCR: pPlayer attempt to unlearn spell %u, but item %u is equipped.",reqSpell, pItem->GetProto()->ItemId);
                return false;
            }
        }
    }
...
bool GossipHello_npc_prof_alchemy(Player* pPlayer, Creature* creature)
{
...
void SendActionMenu_npc_prof_alchemy(Player* pPlayer, Creature* creature, uint32 uiAction)
{
    switch (uiAction)
    {
        ...
        case GOSSIP_ACTION_TRAIN:
            pPlayer->SEND_TRAINERLIST(creature->GetGUID());
            break;
            //Learn Alchemy
...
void SendConfirmLearn_npc_prof_alchemy(Player* pPlayer, Creature* creature, uint32 uiAction)
{
    if (uiAction)
    {
        uint32 eCreature = creature->GetEntry();
...
void SendConfirmUnlearn_npc_prof_alchemy(Player* pPlayer, Creature* creature, uint32 uiAction)
{
    if (uiAction)
    {
        uint32 eCreature = creature->GetEntry();
...
bool GossipSelect_npc_prof_alchemy(Player* pPlayer, Creature* creature, uint32 uiSender, uint32 uiAction)
{
    switch (uiSender)
    {
        case GOSSIP_SENDER_MAIN:    SendActionMenu_npc_prof_alchemy(pPlayer, creature, uiAction); break;
        case GOSSIP_SENDER_LEARN:   SendConfirmLearn_npc_prof_alchemy(pPlayer, creature, uiAction); break;
        case GOSSIP_SENDER_UNLEARN: SendConfirmUnlearn_npc_prof_alchemy(pPlayer, creature, uiAction); break;
        case GOSSIP_SENDER_CHECK:   SendActionMenu_npc_prof_alchemy(pPlayer, creature, uiAction); break;
    }
    return true;
}
...
bool GossipHello_npc_prof_blacksmith(Player* pPlayer, Creature* creature)
{
    if (creature->isQuestGiver())
        pPlayer->PrepareQuestMenu(creature->GetGUID());
    if (creature->isVendor())
...
void SendActionMenu_npc_prof_blacksmith(Player* pPlayer, Creature* creature, uint32 uiAction)
{
    switch (uiAction)
    {
        case GOSSIP_ACTION_TRADE:
...
void SendConfirmLearn_npc_prof_blacksmith(Player* pPlayer, Creature* creature, uint32 uiAction)
{
    if (uiAction)
    {
        uint32 eCreature = creature->GetEntry();
...
void SendConfirmUnlearn_npc_prof_blacksmith(Player* pPlayer, Creature* creature, uint32 uiAction)
{
    if (uiAction)
    {
        uint32 eCreature = creature->GetEntry();
...
bool GossipSelect_npc_prof_blacksmith(Player* pPlayer, Creature* creature, uint32 uiSender, uint32 uiAction)
{
    switch (uiSender)
    {
        case GOSSIP_SENDER_MAIN:    SendActionMenu_npc_prof_blacksmith(pPlayer, creature, uiAction); break;
        case GOSSIP_SENDER_LEARN:   SendConfirmLearn_npc_prof_blacksmith(pPlayer, creature, uiAction); break;
        case GOSSIP_SENDER_UNLEARN: SendConfirmUnlearn_npc_prof_blacksmith(pPlayer, creature, uiAction); break;
        case GOSSIP_SENDER_CHECK:   SendActionMenu_npc_prof_blacksmith(pPlayer, creature, uiAction); break;
    }
    return true;
}

/*bool QuestComplete_npc_prof_blacksmith(Player* pPlayer, Creature* creature, Quest const *_Quest)
{
    if ((_Quest->GetQuestId() == 5283) || (_Quest->GetQuestId() == 5301))             //armorsmith
        creature->CastSpell(pPlayer, 17451, true);

    if ((_Quest->GetQuestId() == 5284) || (_Quest->GetQuestId() == 5302))             //weaponsmith
        creature->CastSpell(pPlayer, 17452, true);
...
bool GossipHello_npc_prof_leather(Player* pPlayer, Creature* creature)
{
    if (creature->isQuestGiver())
        pPlayer->PrepareQuestMenu(creature->GetGUID());
    if (creature->isVendor())
...
void SendActionMenu_npc_prof_leather(Player* pPlayer, Creature* creature, uint32 uiAction)
{
    switch (uiAction)
    {
        case GOSSIP_ACTION_TRADE:
...
void SendConfirmUnlearn_npc_prof_leather(Player* pPlayer, Creature* creature, uint32 uiAction)
{
    if (uiAction)
    {
        uint32 eCreature = creature->GetEntry();
...
bool GossipSelect_npc_prof_leather(Player* pPlayer, Creature* creature, uint32 uiSender, uint32 uiAction)
{
    switch (uiSender)
    {
        case GOSSIP_SENDER_MAIN:    SendActionMenu_npc_prof_leather(pPlayer, creature, uiAction); break;
        case GOSSIP_SENDER_UNLEARN: SendConfirmUnlearn_npc_prof_leather(pPlayer, creature, uiAction); break;
        case GOSSIP_SENDER_CHECK:   SendActionMenu_npc_prof_leather(pPlayer, creature, uiAction); break;
    }
    return true;
}
...
bool GossipHello_npc_prof_tailor(Player* pPlayer, Creature* creature)
{
    if (creature->isQuestGiver())
        pPlayer->PrepareQuestMenu(creature->GetGUID());
    if (creature->isVendor())
...
void SendActionMenu_npc_prof_tailor(Player* pPlayer, Creature* creature, uint32 uiAction)
{
    switch (uiAction)
    {
        case GOSSIP_ACTION_TRADE:
...
void SendConfirmLearn_npc_prof_tailor(Player* pPlayer, Creature* creature, uint32 uiAction)
{
    if (uiAction)
    {
        uint32 eCreature = creature->GetEntry();
...
void SendConfirmUnlearn_npc_prof_tailor(Player* pPlayer, Creature* creature, uint32 uiAction)
{
    if (uiAction)
    {
        uint32 eCreature = creature->GetEntry();
...
bool GossipSelect_npc_prof_tailor(Player* pPlayer, Creature* creature, uint32 uiSender, uint32 uiAction)
{
    switch (uiSender)
    {
        case GOSSIP_SENDER_MAIN:    SendActionMenu_npc_prof_tailor(pPlayer, creature, uiAction); break;
        case GOSSIP_SENDER_LEARN:   SendConfirmLearn_npc_prof_tailor(pPlayer, creature, uiAction); break;
        case GOSSIP_SENDER_UNLEARN: SendConfirmUnlearn_npc_prof_tailor(pPlayer, creature, uiAction); break;
        case GOSSIP_SENDER_CHECK:   SendActionMenu_npc_prof_tailor(pPlayer, creature, uiAction); break;
    }
    return true;
}
